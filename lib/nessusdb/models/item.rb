		...
		class Item < ActiveRecord::Base
			...
			class << self
				...
				def high_risks_unique
					where(:severity => 2).joins(:plugins).order(:cvss_base_score).group(:plugin_id)
				end
				...
				def medium_risks_unique
					where(:severity => 1).joins(:plugins).order(:cvss_base_score).group(:plugin_id)
				end
				...
				def low_risks_unique
					where(:severity => 0).joins(:plugins).order(:cvss_base_score).group(:plugin_id)
				end
				...
				def risks_by_service
					select("items.*").select("count(*) as count_all").where("svc_name != 'unknown' and svc_name != 'general'").group(:svc_name).order("count_all DESC")
				end
				...
				#
				def risks_by_plugin
					select("items.*").select("count() as count_all").where("plugin_id != 1").where(:severity => 3).group(:plugin_id).order("count_all DESC")
				end
				...
				#
				def risks_by_host
					select("items.*").select("count(*) as count_all").where("plugin_id != 1").where(:severity => [3,2]).group(:host_id).order("count_all DESC")
				end
			end
		end
	end
